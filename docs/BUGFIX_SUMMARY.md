...
### 症状
游戏运行正常，但到 7 级升级时，升级面板没有任何选项可选。

### 根本原因

**默认升级池太小**

默认配置只有 7 个升级（3 武器 + 3 升级 + 1 道具），减去道具后只有 6 个可选升级。考虑到前置条件和已收集的升级，到 7 级时升级池已耗尽。
...
这确保玩家在整个游戏流程（5 分钟，约 20-30 级）中始终有升级可选。

### Git 提交

- **提交**: `698c20c`
- **消息**: "fix: 扩展默认升级池，支持 30+ 级游戏"
- **状态**: ✅ 已推送到 origin/main
