...
func spawn_attack_effect(attack_position: Vector2, attack_direction: Vector2):
	if player:
		...
		rect.size = Vector2(attack_range, attack_range * 0.6)
		shape.shape = rect
		shape.rotation = attack_direction.angle()
		hit_box.add_child(shape)
		
		if player.get_parent():
			player.get_parent().add_child(hit_box)
		
		hit_box.area_entered.connect(_on_attack_hit.bind(damage, knockback, attack_direction))
		
		_play_slash_effect(attack_position, attack_direction)
...
func _play_slash_effect(effect_position: Vector2, effect_direction: Vector2):
	...
	effect_node.rotation = effect_direction.angle()
