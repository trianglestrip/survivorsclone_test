...
func _test_visual_standards_config() -> void:
	...
	var std = get_node_or_null("/root/VisualEffectsStandard")
...
func _test_animation_enhancement() -> void:
	print("[测试4] 动画帧增强验证")
	var backup_dir = "Textures/Skills/Animations/backup/"
	var work_dir = "Textures/Skills/Animations/"
	
	_assert(DirAccess.dir_exists_absolute("res://" + backup_dir), "备份目录存在")
	_assert(DirAccess.dir_exists_absolute("res://" + work_dir), "工作目录存在")
	
	var test_files = [
		"ice_shard_frame_0.png",
		"thunder_strike_frame_0.png",
		"fire_ball_frame_0.png",
		"poison_dart_frame_0.png"
	]
	
	for filename in test_files:
		var work_path = work_dir + filename
		_assert(ResourceLoader.exists("res://" + work_path), "增强后文件可加载: " + filename)
